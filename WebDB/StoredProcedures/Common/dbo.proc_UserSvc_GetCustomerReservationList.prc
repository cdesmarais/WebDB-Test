if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[proc_UserSvc_GetCustomerReservationList]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[proc_UserSvc_GetCustomerReservationList]
GO

CREATE Procedure dbo.proc_UserSvc_GetCustomerReservationList
 (
  @UserID int
 )
As
...
GRANT EXECUTE ON [proc_UserSvc_GetCustomerReservationList] TO ExecuteOnlyRole
GO
