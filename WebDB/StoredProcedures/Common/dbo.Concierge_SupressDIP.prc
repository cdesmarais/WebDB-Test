if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[Concierge_SupressDIP]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[Concierge_SupressDIP]
GO

-- Apply DIP supression rules 
CREATE PROCEDURE dbo.Concierge_SupressDIP
(
	@RID int,
...
-- send result back
select SuppressDIP=@theResult
GO

GRANT EXECUTE ON [Concierge_SupressDIP] TO ExecuteOnlyRole
