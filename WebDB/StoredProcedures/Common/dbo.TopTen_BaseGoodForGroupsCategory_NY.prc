...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[TopTen_BaseGoodForGroupsCategoryNY]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[TopTen_BaseGoodForGroupsCategoryNY]
go
...
/*
	...
	Special proc for NY to exclude the following regions 
	Connecticuit = 23
	Upstate New York = 77
	New Jersey Central = 160
*/

create procedure dbo.TopTen_BaseGoodForGroupsCategoryNY
(
...
where (MA.active = 1 or MA.MetroAreaID=1) /*explicitly added Demoland metroarea*/ 
    ...
    and MOOP.MaxValue >= 6 /*Min party size for online reservations*/
...
grant execute on [TopTen_BaseGoodForGroupsCategoryNY] to ExecuteOnlyRole
go
