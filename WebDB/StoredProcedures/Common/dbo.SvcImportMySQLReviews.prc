...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SvcImportMySQLReviews]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].SvcImportMySQLReviews
GO

-- Drop type after reference is removed
if exists(select * from sys.types where name = 'MySqlReviewsImportType' and is_table_type = 1)
drop type dbo.MySqlReviewsImportType
go

CREATE TYPE dbo.MySqlReviewsImportType AS TABLE
(
	ResID			int not null, 
...
grant execute on type::dbo.MySqlReviewsImportType to ExecuteOnlyRole
go

CREATE PROCEDURE dbo.SvcImportMySQLReviews
(
	@MySqlDataSet dbo.MySqlReviewsImportType READONLY
)
AS
...
set @ProcName = 'MySqlReviewsImportType'
...
grant execute on [dbo].SvcImportMySQLReviews TO ExecuteOnlyRole
