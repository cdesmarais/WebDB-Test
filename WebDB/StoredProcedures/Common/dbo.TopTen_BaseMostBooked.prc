...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[TopTen_BaseMostBooked]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[TopTen_BaseMostBooked]
go

/*
	Used as the base procedure to generate most-booked type of lists. Content owned by India team, 
	please notify asaxena@opentable.com if changing.
*/

create procedure dbo.TopTen_BaseMostBooked 
(
...
grant execute on [TopTen_BaseMostBooked] to ExecuteOnlyRole
go
