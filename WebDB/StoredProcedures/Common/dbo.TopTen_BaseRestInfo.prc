...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[TopTen_BaseRestInfo]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[TopTen_BaseRestInfo]
go
...
create procedure dbo.TopTen_BaseRestInfo
(
...
grant execute on [TopTen_BaseRestInfo] to ExecuteOnlyRole
go
